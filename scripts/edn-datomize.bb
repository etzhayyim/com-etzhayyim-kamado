...
;;  com-etzhayyim-kamado 用に adapt: schema.edn はこのリポジトリ root に生成)。
;;
...
;;
;; 使い方 (com-etzhayyim-kamado 用に adapt: schema.edn はリポジトリ root に置く。
;; manifest/ ディレクトリが無い子リポ向け):
;;   bb scripts/edn-datomize.bb wrap-map <path> <ns>         — map 1個のファイルを変換（ns で bare key を prefix）
;;   bb scripts/edn-datomize.bb wrap-map-preserve <path> <ns> — map 1個のファイルを変換（既に namespaced な key はそのまま、bare key だけ ns を prefix）
...
;; ---------- namespace-preserving wrap (idiomatic-key files, e.g. :actor/id) ----------
;; ファイルのキーが既に意味のある名前空間を持つ場合（例: :actor/id）、その名前空間を
;; 壊さず維持し、名前空間の無い bare key だけ ns-name を付与する。

(defn- preserve-key [ns-name k]
  (if (namespace k) k (namespaced-key ns-name k)))

(defn entity-from-map-preserve [content ns-name]
  (into {:db/id -1}
        (map (fn [[k v]] [(preserve-key ns-name k) (attr-value v)]))
        content))

(defn schema-attrs-preserve [content ns-name]
  (for [[k v] content]
    (let [{:keys [type card]} (classify v)]
      {:db/ident (preserve-key ns-name k)
       :db/valueType type
       :db/cardinality card})))
  ...
  (let [f (io/file root rel-path)
    ...
    (if (already-tx-data? content)
      ...
      (let [entity (entity-from-map-preserve content ns-name)
            attrs (schema-attrs-preserve content ns-name)]
        (spit f (pr-str [entity]))
        (merge-schema! attrs)
        (println "wrapped (ns-preserving)" rel-path "->" (count entity) "attrs, default-ns=" ns-name)))))
  ...
  (let [[mode a b] args]
    (case mode
      ...
      "adr-file" (let [report (atom {:ok [] :skipped [] :errors [] :attrs []})]
                   ...
                   (merge-schema! (:attrs @report))
                   (println @report))
      (do (println "usage: bb scripts/edn-datomize.bb [wrap-map <path> <ns> | wrap-map-preserve <path> <ns> | adr-dir <dir> | adr-file <path>]")
          (System/exit 1)))))
